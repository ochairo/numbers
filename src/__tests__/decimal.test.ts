...
describe("Decimal", () => {
  ...
  describe("arithmetic operations", () => {
    ...
    it("should divide correctly when dividend has a fractional part", () => {
      expect(Decimal("1.5").divide("2").toString()).toBe("0.75");
      expect(Decimal("10.5").divide("2").toString()).toBe("5.25");
      expect(Decimal("-1.5").divide("2").toString()).toBe("-0.75");
    });

    it("should divide correctly when divisor has a fractional part", () => {
      expect(Decimal("1").divide("0.5").toString()).toBe("2");
      expect(Decimal("3").divide("1.5").toString()).toBe("2");
      expect(Decimal("1").divide("0.3", 5).toString()).toBe("3.33333");
    });

    it("should divide correctly when both operands have fractional parts", () => {
      expect(Decimal("1.5").divide("0.5").toString()).toBe("3");
      expect(Decimal("2.25").divide("0.75").toString()).toBe("3");
      expect(Decimal("0.1").divide("0.01").toString()).toBe("10");
    });
    ...
    it("should throw RangeError when precision is too low for the dividend's scale", () => {
      // dividend has scale 1, divisor has scale 0 → min precision = 1
      expect(() => Decimal("1.5").divide("2", 0)).toThrow(RangeError);
      expect(() => Decimal("1.5").divide("2", 0)).toThrow(
        "precision (0) is too low",
      );
      // dividends with scale 2 need precision >= 2 (when divisor has scale 0)
      expect(() => Decimal("1.55").divide("1", 1)).toThrow(RangeError);
      // exactly at the minimum → no throw
      expect(() => Decimal("1.5").divide("2", 1)).not.toThrow();
    });
  ...
  describe("conversions", () => {
    ...
    it("should convert to JSON", () => {
      ...
      expect(JSON.stringify({ value: Decimal("3.14") })).toBe(
        '{"value":"3.14"}',
      );
    });
  ...
  describe("chaining", () => {
    ...
    it("should work with complex expressions", () => {
      const result = Decimal("100").multiply("0.15").add("50").divide("2");
  ...
  describe("edge cases", () => {
    ...
    it("should handle MySQL DECIMAL(65,30) limits", () => {
      ...
      expect(mysqlMax.toString()).toBe(
        "99999999999999999999999999999999999.999999",
      );
